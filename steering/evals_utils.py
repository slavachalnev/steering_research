...
async def run_battle(text_1, text_2, criterion, prompt, client, model):
    ...
    """
    system_message = "You rank two texts generated by a language model based on the following criterion: \n"
    system_message += criterion + "\nYou must choose which text is better based on the criterion. "\
"You must return JSON with two fields: \"winner\" (int) and \"tie\" (bool). The winner field should be 1 or 2."
    prompt_user_text = f"Prompt:\n\n{prompt}\n\n" if prompt else ""
    ...
    try:
        ...
        print(f"JSON decode error: {e}")
        print(f"Text causing the error:\n{text_1}\n{text_2}")
        return {"winner": 0, "tie": False, "error": "Failed to parse JSON response"}
    except Exception as e:
        print(f"Error in evaluate_completion: {e}")
        return {"winner": 0, "tie": False, "error": str(e)}
...
def compute_battles(
    ...
    async def evaluate_all():
        tasks = [
            ...
            for text_1, text_2, _, _ in battles
        ]
        # chunk tasks into batches
        chunked_tasks = [tasks[i:i+batch_size] for i in range(0, len(tasks), batch_size)]
        results = []
